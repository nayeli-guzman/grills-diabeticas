...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, fill=blue!8, minimum width=2.6cm, minimum height=1.1cm, align=center, font=\footnotesize},
    arr/.style={-{Stealth[length=2mm]}, thick}
]
\node[box] (adq) {1. Adquisición\\(EHR)};
\node[box, right=of adq] (pred) {2. Predictivo\\adaptativo};
\node[box, right=of pred] (inc) {3. Incertidumbre\\\& calibración};
\node[box, right=of inc] (dec) {4. Decisión\\\& umbrales};
\node[box, right=of dec] (acc) {5. Acción\\clínica (HITL)};
\node[box, below=1.1cm of pred, fill=orange!12] (drift) {6. Monitoreo de\\data/concept drift};

\draw[arr] (adq) -- (pred);
\draw[arr] (pred) -- (inc);
\draw[arr] (inc) -- (dec);
\draw[arr] (dec) -- (acc);
\draw[arr] (drift.west) -| ([xshift=-0.3cm]adq.south) -- (adq.south);
\draw[arr] (pred.south) -- (drift.north);
\draw[arr] (drift.east) -| ([xshift=0.3cm]dec.south) -- (dec.south);
\end{tikzpicture}
\caption{Arquitectura del sistema: seis módulos, con retroalimentación del Módulo de Monitoreo de Drift hacia el reentrenamiento y hacia el ajuste de umbrales de decisión.}
...
Antes de reportar cualquier métrica de drift o de negocio, el equipo auditó la preparación de datos (\texttt{02\_EDA.ipynb}, Sección 10) siguiendo una batería de trece pruebas de fuga de información (disponibilidad de variables al alta, cribado univariado, identificadores, separación perfecta, latencia de etiqueta, contaminación entre conjuntos, etiquetas permutadas, desempeño plausible, pacientes vistos vs.\ nuevos, variables de contexto, dominancia de variables y reproducibilidad). La auditoría completa está en \texttt{artifacts/data\_preparada/auditoria\_leakage.csv}.

Esta auditoría dejó dos recomendaciones explícitas para el modelado (\texttt{02\_EDA.ipynb}, conclusión \S10.7) que, al revisar \texttt{03\_modelado.ipynb} y \texttt{05\_drift\_adaptacion.ipynb} para este informe, \textbf{no se habían aplicado}:
...
La pregunta que valida todo el trabajo de las secciones anteriores es simple: \textbf{¿vale la pena construir y mantener el módulo de monitoreo de drift, o basta con desplegar el modelo una vez y olvidarlo?} Para responderla se comparan, sobre los mismos bloques de prueba, dos políticas:

\begin{itemize}[leftmargin=*]
    \item \textbf{Sin monitoreo (``desplegar y olvidar''):} el modelo se entrena una sola vez (bloques 1--2) y los umbrales de decisión $t_1,t_2$ se calibran una sola vez al inicio, sin volver a mirarse.
    \item \textbf{Con monitoreo (estrategia adaptativa elegida):} el modelo y sus umbrales se actualizan según la estrategia con mejor PR-AUC en los bloques comunes (Sección~\ref{sec:drift}), disparada por el módulo de monitoreo de drift.
\end{itemize}

Para cada política se calcula el costo esperado por cada 1,000 altas (fórmula de costo esperado de la Sección anterior) en cada bloque de prueba, y se proyecta a una escala anual. Como el dataset no reporta el volumen anual de un hospital específico —solo el agregado de 10 años en 130 hospitales \cite{strack2014}— se usa como escala ilustrativa \supuesto{ALTAS\_ANUALES = 50{,}000 altas/año, del orden de una red hospitalaria mediana-grande}.
...
El costo de mantenimiento se construye de abajo hacia arriba (monitoreo mensual de PSI/KS/calibración por subgrupo + cómputo batch, más el costo de cada reentrenamiento disparado por el módulo de drift), siguiendo el argumento de \cite{sculley2015} de que en sistemas de ML reales el costo dominante no es el entrenamiento inicial sino el monitoreo y el ``glue code'' de mantenimiento continuo — por eso este caso de negocio no asume mantenimiento gratuito.

\textbf{Beneficio adicional no cuantificado (conservador):} un hospital sujeto al HRRP enfrenta penalizaciones económicas directas por exceso de reingresos \cite{vanwalraven2011}; reducir reingresos evitables reduce también esa exposición regulatoria, un beneficio que no se incluyó en el cálculo anterior para mantenerlo conservador.
